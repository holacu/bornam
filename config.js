...
const config = {
    ...
    telegram: {
        ...
        security: {
            ...
            maxRequestsPerWindow: parseInt(process.env.RATE_LIMIT_MAX_REQUESTS) || 30, // عدد الطلبات المسموحة في النافزة الزمنية
            adminOnly: process.env.ADMIN_ONLY_MODE === 'true' // هل النظام مقتصر على الأدمن فقط
        }
    },
    ...
    bots: {
        ...
        reconnection: {
            maxAttempts: parseInt(process.env.MAX_RECONNECT_ATTEMPTS) || 5, // عدد محاولات إعادة الاتصال
        ...
        // مهلة الاتصال
        timeout: {
            connection: (parseInt(process.env.CONNECTION_TIMEOUT) || 30) * 1000, // مهلة الاتصال
        ...
        // إعدادات الأداء
        performance: {
            maxMemoryUsage: (parseInt(process.env.MAX_MEMORY_USAGE) || 512) * 1024 * 1024, // MB إلى bytes
            gcInterval: parseInt(process.env.GC_INTERVAL) || 300000, // فترة تنظيف الذاكرة (5 دقائق)
            statsUpdateInterval: parseInt(process.env.STATS_UPDATE_INTERVAL) || 60000 // فترة تحديث الإحصائيات (دقيقة)
        }
    },
    ...
    database: {
        ...
        path: process.env.DATABASE_PATH || './minecraft_bot.db',
    ...
    logging: {
        ...
        bots: {
            logChat: process.env.LOG_CHAT !== 'false', // تسجيل رسائل الشات
            logCommands: process.env.LOG_COMMANDS !== 'false', // تسجيل الأوامر
